    ...
    try {
      ...
      if (result.success) {
        setStories((prevStories) =>
          prevStories.map((story) => (story.id === newStory.id ? { ...story, ...newStory } : story))
        );
        setOpenModal(false);
        setNewStory({
          id: null,
  ...
  const handleOpenModal = (story = null) => {
    if (story) {
      setNewStory({
        ...story,
        creator_name:
          creators.find((creator) => creator.id === story.creator_id)?.creatorName || "",
        ...
        <DialogContent>
          ...
          <Autocomplete
            ...
            onChange={(event, value) =>
              setNewStory({ ...newStory, creator_id: value?.id, creator_name: value?.creatorName })
            }
            renderInput={(params) => (
          ...
          <Autocomplete
            ...
            onChange={(event, value) =>
              setNewStory({ ...newStory, genre_id: value?.id, genre_name: value?.genre_name })
            }
            renderInput={(params) => (
